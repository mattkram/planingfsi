...
class PotentialPlaningSolver(object):
    ...
    def __init__(self):
        ...
        self.lineFS = None
        self.lineFSi = None
#         PotentialPlaningSolver.obj.append(self)
        self.solver = None
    ...
    def calculate_pressure(self):
        ...
        unknownEl = [
            el for el in self.pressure_elements
            if not el.is_source and el.width > 0.0]
        ...
        A = np.array([el.get_influence_coefficient(x)
        ...
        if not dim == 0:
            p = np.linalg.solve(
                np.reshape(A, (dim, dim)), np.reshape(Z, (dim, 1)))[:, 0]
        else:
            p = np.zeros_like(Z)
    ...
    def plot_residuals_over_range(self):
        ...
        for i, Zi, seal in zip([2, 3], [Z1, Z2], ['bow', 'stern']):
            ...
            plt.title(
                'Residual for {0} seal trailing edge pressure'.format(seal))
            plt.xlabel('Bow seal length')
            plt.ylabel('Stern seal length')
            plt.savefig(
                '{0}SealResidual_{1}.png'.format(seal, config.it),
                format='png')
            plt.clf()
...
# kp.growPoints(ptsR[1],  ptsR[0],  xEnd, config.growthRate))
            ...
            if len(xFS) > 0:
                ...
                self.zFS = np.array(
                    map(self.get_free_surface_height, self.xFS))
            else:
    ...
    def get_free_surface_height(self, x):
        ...
        """
        return sum([patch.get_free_surface_height(x)
    ...
    def get_free_surface_derivative(self, x, direction='c'):
        ...
        """
        ddx = kp.getDerivative(self.get_free_surface_height, x,
                               direction=direction)
        return ddx
